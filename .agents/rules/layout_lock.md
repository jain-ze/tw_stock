...
>    - **[防範 7] 每日行情持久化快取資料庫 (etf_history_cache.json) 每日執行與 6 日保留規範**：
      1. **持久化檔案儲存**：在系統根目錄建立 `etf_history_cache.json`（並同步至 `近5日台股成交金額TOP20深度分析/`），自動保存 292 檔上市/上櫃 ETF 歷史上每一天不重複的真實行情（收盤價、漲跌、成交股數、成交金額、筆數、開高低價），原則上必須保留至少前 6 個交易日以上之真實歷史數據。
      2. **歷史日期動態調用**：對上櫃 ETF（如 00411A），系統僅對「當天 (`d_str == today_str`)」向櫃買 API 拿最新快照並寫入快取；「歷史交易日 (`d_str != today_str`)」則嚴禁發起 API 查詢或使用當日快照覆蓋，而是 100% 直接讀取快取中過去累積保存的真實數值。
      3. **每日自動備份**：每次排程執行完畢，`etf_history_cache.json` 會隨同每日報告自動推播至 GitHub 儲存庫 (`jain-ze/tw_stock.git`)，確保伺服器重啟或換環境數據永遠不遺失。
>    - **[防範 8] 上櫃 ETF 完整性熔斷與 18 檔強制驗證斷言**：`build_etf_dataset.py` 必須內建 18 檔上櫃股票型/主動式 ETF 必備清單 (`00411A`, `006201`, `00858`, `00877`, `00886`, `00887`, `00888`, `00928`, `00955`, `009806`, `009807`, `00980T`, `009814`, `009815`, `009822`, `009823`, `009825`, `00998A`)。每次產出前必須強制進行 100% 完整性斷言，若檢測到任何 1 檔遺漏或歷史交易日少於 5 日，腳本必須立即中斷熔斷並觸發重試機制，確保每次執行均 100% 完整無虞。
>    - **[防範 9] 上櫃股票歷史 API 鎖定規範**：櫃買中心 (TPEx) 舊版舊 API `stk_quote_result.php` 帶入歷史日期參數 `d=` 時，伺服器會靜默忽視該參數並恆定回傳「當日最新快照」，導致上櫃股票（環球晶 6488、群聯 8299、合晶 6182 等）歷史 5 日數據呈現相同靜態數值。系統必須鎖定並使用櫃買中心官方最新 API `https://www.tpex.org.tw/www/zh-tw/afterTrading/dailyQuotes?date={ROC_YEAR}/{MM}/{DD}&response=json`，確保上櫃個股每一天的歷史收盤價、成交股數、成交金額均為真實獨立數據。
