...
print("1. Fetching ETF Basic Info Metadata from TWSE & TPEx OpenAPI...")
...
def fetch_valid_trading_days(n=6):
    ...
    sub_cache_file = os.path.join(SUB_DIR, 'etf_history_cache.json')
    etf_history_cache = {}
    
    for c_path in [cache_file, sub_cache_file]:
        if os.path.exists(c_path):
            try:
                with open(c_path, 'r', encoding='utf-8') as f:
                    loaded = json.load(f)
                    for code, hdict in loaded.items():
                        etf_history_cache.setdefault(code, {}).update(hdict)
    ...
    while len(valid_dates) < n and attempts < 45:
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if stat == 'OK' and data.get('data'):
                        # Normalize ETFDaily format to standard schema: [Code, Name, Volume, Count, Value, Open, High, Low, Close, Dir, Diff]
                        norm_rows = []
                        for r in data['data']:
                            code = r[0].strip()
                            if code.startswith('00'):
                                ...
                                cp = r[8]
                                diff_str = r[9]
                                d_sym = '+' if str(diff_str).startswith('+') else ('-' if str(diff_str).startswith('-') else '')
                                norm_rows.append([code, r[1], vol, cnt, val, op, hp, lp, cp, d_sym, diff_str])
                        if norm_rows:
                            fetched_data = norm_rows
                            break
        ...
        if fetched_data is not None:
            ...
            # 🛡️ 歷史日期動態調用與持久化快取保護機制：
            # 上櫃 ETF (如 00411A) 僅在「當天」(d_str == today_str) 發起 API 查詢並更新快取；
            # 歷史交易日 (d_str != today_str) 嚴禁發起 API 查詢 (防止 TPEx 伺服器回傳當日快照覆蓋歷史數值)，
            # 必須 100% 直接讀取 etf_history_cache.json 中過去累積保存之真實行情。
            today_str = datetime.date.today().strftime('%Y%m%d')
            is_today = (d_str == today_str) or (len(valid_dates) == 0)
